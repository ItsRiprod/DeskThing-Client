    ...
    <div className="p-6 h-full grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 overflow-y-auto">
      <section className="m-2 bg-zinc-900 rounded-lg p-4 shadow-lg">
        ...
        <div className="space-y-2">
          ...
          <div className="text-gray-300">
            Server: {manifest.ip}:{manifest.port}
          </div>
        </div>
      </section>
      ...
      <section className="m-2 bg-zinc-900 rounded-lg p-4 shadow-lg">
        ...
        <div className="space-y-2">
          ...
          <Button onClick={handleRestartOnboarding} className="text-gray-300">
            Restart Onboarding
          </Button>
        </div>
      </section>
      ...
      <section className="m-2 bg-zinc-900 rounded-lg p-4 shadow-lg">
        ...
        <div className="space-y-1 max-h-40 overflow-y-auto">
          {logs.slice(-5).map((log, index) => (
            <div
              key={index}
              className={`text-sm ${log.type === 'error' ? 'text-red-400' : 'text-gray-300'}`}
            >
              [{log.app}] {log.payload}
            </div>
          ))}
